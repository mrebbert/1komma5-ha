...
| System | Site + per-asset connectivity binaries, the three "active feature" binaries (dynamic tariff / time-of-use / smart charging), all seven diagnostic update-timestamp sensors (live / price / optimization / weather / system_status / energy / notification), and the EMS auto-mode switch (diagnostic-categorised since v0.1.46, only created on GRIDX-backend installs from v0.1.57 on). |
