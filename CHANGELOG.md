...
- Bumped the `onekommafive` SDK pin to `>=1.0.3,<2` (from `>=0.5.0,<0.6`). v1.0.2 loosened the aiohttp constraint to `>=3.10,<4`, matching Home Assistant's own aiohttp 3.13.3 requirement; v1.0.3 is the current maintenance release picked up automatically at install time.
