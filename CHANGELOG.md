...
### Changed
- Bump the `onekommafive` SDK pin to `>=0.1.25,<0.2` (from `>=0.1.23,<0.2`). Upstream `0.1.24` was a maintenance release only: hardened Auth0 login-state parsing (regex tolerates whitespace/attribute-order variations in the response template) plus internal DRY refactors. Upstream `0.1.25` adds a new endpoint — `system.get_energy_savings(from_date, to_date)` returning aggregated Heartbeat savings (€) for an arbitrary date range (`GET /v1/systems/{sys}/energy-savings`). The new endpoint is available to consumers but not yet surfaced as a Home Assistant entity — see the roadmap. Every existing entity keeps its `unique_id`, `entity_id` and translation key. Verified locally (unit + integration + mypy + ruff) and smoke-tested against a live system.
