...
### Changed
- Bump the `onekommafive` SDK pin to `>=0.1.24,<0.2` (from `>=0.1.23,<0.2`). Upstream `0.1.24` is a low-risk maintenance release: hardened Auth0 login-state parsing (regex tolerates whitespace/attribute-order variations in the response template) and internal DRY refactors (`_describe_weather` helper, CLI `_get_system` helper). No API-surface changes for consumers; every entity keeps its `unique_id`, `entity_id` and translation key. Verified locally (unit + integration + mypy + ruff) and smoke-tested against a live system.
